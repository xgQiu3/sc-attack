...
class FlowVAE(Module):
    ...
    def sample(self, w, num_points, flexibility, bert, victim_model,
               cls_loss, label, k, truncate_std=None):
        batch_size, _ = w.size()
        if truncate_std is not None:
            w = truncated_normal_(w, mean=0, std=1, trunc_std=truncate_std)
        # Reverse: z <- w.
        z = self.flow(w, reverse=True).view(batch_size, -1)
        samples = self.diffusion.sample(num_points, z, bert, victim_model, cls_loss, k, label,
                                        point_dim=3, flexibility=flexibility)
        return samples
